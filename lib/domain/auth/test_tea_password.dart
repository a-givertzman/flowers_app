...
void main() {
  ...
  print('escaped(${escaped.length}): $escaped');
  final unescaped = _unescape(escaped);
  print('unescaped(${unescaped.length}): $unescaped');
  if (str == unescaped) {
    print('ok');
  }
  ...
  for (final value in values) {
    ...
    print('encrypted(${encrypted.length}): $encrypted');
    final decrypted = _decrypt(encrypted, key);
    print('decrypted(${decrypted.length}): $decrypted');
    if (value == decrypted) {
      print('\tOK');
    } else {
      err++;
...
String _encrypt(String source, String key) {
  ...
  for (var i = 0; i < source.length; i += 8) {  // encode plaintext into s in 64-bit (8 char) blocks
    ...
    // print('encode slicedV0: $slicedV0');
    // print('encode slicedV1: $slicedV1');
    // print('encode V0: ${v[0]}');
    // print('encode V1: ${v[1]}');
    encode(v, k);
    // print('longToStr4(v[0]): ${longToStr4(v[0])}');
    // print('longToStr4(v[1]): ${longToStr4(v[1])}');
...
List decode(List v, List k) {
  ...
  while (sum != 0) {
    final add2 = int.parse('${k[sum >>> 11 & 3]}');
    // print('decode add2: $add2');
    z -= ((y << 4) ^ (y >>> 5)) + (y ^ sum) + add2;
    sum -= delta;
    final add1 = int.parse('${k[sum & 3]}');
    // print('decode add1: $add1');
    y -= ((z << 4) ^ (z >>> 5)) + (z ^ sum) + add1;
  }
  v[0] = y; 
  v[1] = z;
